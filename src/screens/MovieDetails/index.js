...
const MovieDetails = ({ navigation, route }) => {
  ...
  const handleShare = (title, id) => {
    if (isError) {
      Alert({
        title: 'Perhatian',
        description: 'Telah terjadi kesalaha, coba lagi!.'
      });
    } else {
      Share({
        message: `${title}, tahu semuanya tentang movie ini \u{1F37F}`,
        url: `https://www.themoviedb.org/movie/${id}`,
        title: 'NCinema',
        dialogTitle: `${title}, tahu semuanya tentang movie ini \u{1F37F}`
      });
    }
  };
                ...
                <SectionRow title="Sinopsis">
                  <ReadMore
                    ...
                    renderTruncatedFooter={(handlePress) =>
                      renderReadMoreFooter('Baca lebih banyak', handlePress)
                    }
                    renderRevealedFooter={(handlePress) =>
                      renderReadMoreFooter('Baca lebih sedikit', handlePress)
                    }
                  >
